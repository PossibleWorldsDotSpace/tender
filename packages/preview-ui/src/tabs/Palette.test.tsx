  ...
  vi.stubGlobal("fetch", vi.fn((url: string) => {
    if (url === "/_api/palette") {
      return Promise.resolve(new Response(JSON.stringify({
        components: [
          { name: "callout", meta: { tag: "aside", class: "callout" },
...
describe("Palette", () => {
  ...
  it("hoists @font-face rules to document head", async () => {
    vi.stubGlobal("fetch", vi.fn((url: string) => {
      if (url === "/_api/palette") {
        return Promise.resolve(new Response(JSON.stringify({ components: [], typography: [] }), { status: 200 }));
      }
      if (url === "/_api/styles.css") {
