...
// The interactive picker (`editTokens` / `tender tokens edit`) was retired
// in #17 slice 4 — the token picker now lives in `tender configure`
// (config-edit/token-picker*) alongside page setup, with the mandatory
// diff-before-write. This module keeps only the non-interactive surfaces:
// `tokens list` and `tokens set`.
