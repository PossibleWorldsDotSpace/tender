...
function reduceList(state: PageSetupState, key: KeyEvent): PageSetupState {
  ...
  // "Nothing changed" confirm screen was misleading users into thinking
  // they'd hit a wall).
  if (key.str === "n") {
...
function toConfirm(state: PageSetupState): PageSetupState {
  // Same short-circuit as the list-view `n` shortcut: with no edits, skip
  // the dead-end "Nothing changed" confirm screen and resolve as no-op so
  // the surrounding init flow advances.
  if (collectEdits(state).length === 0) {
    return { ...state, phase: "done" };
  }
  return { ...state, phase: "confirm", status: "" };
