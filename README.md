...
- **`tender init [dir]`** — scaffold a new project from the default starter (idempotent; preserves existing files) and write a `.gitignore`. On a terminal it prompts: install the tender-author Claude skill (default yes), `git init` (default yes), keep `out/` under version control (default no), then optionally open page setup and the design-token picker (both default no). Each prompt has a skip flag (`--skill`/`--no-skill`, `--git`/`--no-git`, `--track-out`/`--ignore-out`, `--configure-page`/`--no-configure-page`, `--configure-tokens`/`--no-configure-tokens`). Non-interactive defaults: git yes, skill no, `out/` ignored, no configurator. It then offers an initial commit; `--no-commit` skips that. Pass `--example` (or `--example=open-circle`) to scaffold the worked-example project instead — refuses on conflict, override with `--force`.
...
- **`tender configure [dir]`** — interactively adjust page setup (size, margins) and design tokens against an existing project, re-runnable any time. Each screen ends with a diff you confirm before anything is written; comments and unrelated `project.yaml` keys are preserved. `--page-only` / `--tokens-only` narrow it. TTY required (use `tender tokens set` non-interactively).
